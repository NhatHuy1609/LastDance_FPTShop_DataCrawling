...
class WashingmachineIngestion(BaseIngestion):
    """Lớp nạp dữ liệu washing machine từ FPT Shop."""
    
    def __init__(self, api_url: str, landing_zone_path: str, batch_size: int = 50):
        """
        Khởi tạo lớp nạp dữ liệu washing machine.
        ...
        self.csv_filename = "fpt_washingmachines.csv"
        self.api_endpoint = "WashingMachine"
    ...
    def transform_data(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        """
        Chuyển đổi dữ liệu washing machine từ DataFrame sang định dạng API.
        
        Args:
            df: DataFrame chứa dữ liệu washing machine
        ...
        for _, row in df.iterrows():
            ...
            transformed_data.append(washingmachine)
    ...
    async def ingest_async(self) -> None:
        """Thực thi quá trình nạp dữ liệu washing machine bất đồng bộ."""
        ...
        print(f"Đang gửi {total_records} bản ghi washing machine đến API (bất đồng bộ)")
        ...
        print("Hoàn thành quá trình nạp dữ liệu washing machine (bất đồng bộ)")
