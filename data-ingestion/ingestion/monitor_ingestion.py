...
class MonitorIngestion(BaseIngestion):
    """Lớp nạp dữ liệu màn hình từ FPT Shop."""
    
    def __init__(self, api_url: str, landing_zone_path: str, batch_size: int = 50):
        """
        Khởi tạo lớp nạp dữ liệu màn hình.
        ...
        self.csv_filename = "fpt_monitors.csv"
        self.api_endpoint = "Monitor"
    ...
    def transform_data(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        """
        Chuyển đổi dữ liệu màn hình từ DataFrame sang định dạng API.
        
        Args:
            df: DataFrame chứa dữ liệu màn hình
        ...
        for _, row in df.iterrows():
            ...
            # Xác định trạng thái có hàng (mặc định là có hàng)
            is_available = "true"
            
            # Xác định loại màn hình dựa trên tên sản phẩm
            category = "Monitor"
            if "Gaming" in row['Tên sản phẩm']:
                category = "Gaming Monitor"
            elif "Đồ họa" in row['Tên sản phẩm']:
                category = "Graphic Monitor"
            elif "Cong" in row['Tên sản phẩm']:
            ...
            transformed_data.append(monitor)
    ...
    def read_and_clean_csv(self, filename: str) -> pd.DataFrame:
        ...
        # Loại bỏ các dòng không có giá hoặc url
        if 'Giá hiện tại' in df.columns:
            invalid_count = df['Giá hiện tại'].isna().sum()
            if invalid_count > 0:
                print(f"Phát hiện {invalid_count} bản ghi không có giá")
                df = df.dropna(subset=['Giá hiện tại'])
                print(f"Đã loại bỏ các bản ghi không có giá, còn lại {len(df)} bản ghi")
        ...
        if removed_records > 0:
            print(f"Tổng cộng đã loại bỏ {removed_records} bản ghi trùng lặp hoặc không hợp lệ")
        else:
            print("Không phát hiện bản ghi trùng lặp hoặc không hợp lệ nào")
        
        return df
    
    async def ingest_async(self) -> None:
        """Thực thi quá trình nạp dữ liệu màn hình bất đồng bộ."""
        ...
        print(f"Đang gửi {total_records} bản ghi màn hình đến API (bất đồng bộ)")
        ...
        print("Hoàn thành quá trình nạp dữ liệu màn hình (bất đồng bộ)")
