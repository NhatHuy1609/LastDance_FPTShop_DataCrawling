...
class GamingGearsIngestion(BaseIngestion):
    """Lớp nạp dữ liệu gaming gear từ FPT Shop."""
    
    def __init__(self, api_url: str, landing_zone_path: str, batch_size: int = 50):
        """
        Khởi tạo lớp nạp dữ liệu gaming gear.
        ...
        self.csv_filename = "fpt_gaming_gears.csv"
        self.api_endpoint = "GamingGear"
    ...
    def transform_data(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        """
        Chuyển đổi dữ liệu gaming gear từ DataFrame sang định dạng API.
        
        Args:
            df: DataFrame chứa dữ liệu gaming gear
        ...
        for _, row in df.iterrows():
            ...
            # Xác định trạng thái có hàng
            status = row.get('Trạng thái', 'Có hàng') 
            is_available = status != "Hàng sắp về"
            
            # Tạo đối tượng gaming gear
            gaming_gear = {
                ...
                "category": row.get('Loại', 'Phụ kiện gaming khác'),
                "isAvailable": is_available
            }
            
            transformed_data.append(gaming_gear)
    ...
    async def ingest_async(self) -> None:
        """Thực thi quá trình nạp dữ liệu gaming gear bất đồng bộ."""
        ...
        print(f"Đang gửi {total_records} bản ghi gaming gear đến API (bất đồng bộ)")
        ...
        print("Hoàn thành quá trình nạp dữ liệu gaming gear (bất đồng bộ)")
